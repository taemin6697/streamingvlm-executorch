#!/usr/bin/env python3
"""Plot XNNPACK/Vulkan/QNN memory usage by context length from run logs.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot XNNPACK/Vulkan/QNN memory usage up to 8k context length."
    )
    parser.add_argument(
        "--root",
        type=Path,
...
def summarize_backend(root: Path, backend: str, column: str, metric: str) -> list[dict]:
    ...
    rows: list[dict] = []
    artifact_glob = "internvl3_1b_qnn_*/android_memory_timeline.csv"
    if backend != "qnn":
        artifact_glob = f"internvl3_{backend}_1b_*/android_memory_timeline.csv"
    for csv_path in sorted(log_root.glob(artifact_glob)):
...
def plot_single_backend(rows: list[dict], backend: str, output: Path) -> None:
    import matplotlib.pyplot as plt
    ...
    values = [row["used_mib"] for row in backend_rows]
    ...
    ax.set_xlabel("Sequence Length")
    ax.set_ylabel("Memory Used (MiB)")
    ax.set_title(f"{backend.upper()} Memory Usage by Sequence Length")
    ax.set_xticks(x)
    ax.set_xticklabels([str(row["seq_len"]) for row in backend_rows])
...
def main() -> int:
    args = parse_args()
    rows = []
    for backend in ("xnnpack", "vulkan", "qnn"):
    ...
    for backend in ("xnnpack", "vulkan", "qnn"):
        plot_single_backend(
            rows,
            backend,
            args.output.parent / f"{backend}_memory_by_seq.png",
        )
    print(f"Wrote {args.output}")
    print(f"Wrote {args.csv_output}")
    for backend in ("xnnpack", "vulkan", "qnn"):
        print(f"Wrote {args.output.parent / f'{backend}_memory_by_seq.png'}")
    return 0
